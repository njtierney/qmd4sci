...
Quarto documents can be compiled to PDF, which is a great feature. In order to convert the documents to PDF, they use a software called LaTeX ($\LaTeX$) (pronounced la-tek or lay-tek - the `k` comes from the `X` being chi - $\chi$). 

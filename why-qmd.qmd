...
Reproducibility isn't just something that impacts a few people, and it's not cheap. A 2015 estimate stated that in the biomedical industry, in the USA, irreproducibility (not being able to reproduce a given piece of work) costs \$28 Billion dollars annually [^why-qmd-1]. That was one country, one field, and one year.
...
One of the big things that is different between Rmarkdown and Quarto are the chunk options. In Rmarkdown they are in the chunk header, use `.` as a separator (`fig.height`), and use `=`, and in quarto they are "inside" the code fence, and use `-` as a separator (`fig-height`) and use `:`. 

We talk about this later on in the @sec-using-quarto.
...
````{verbatim, lang = "r"}
```{r name, fig.height=7, fig.width=5}
```
````
...
Note that Rmarkdown now also takes in these "hash-pipe" (`#|`) options.

You can automatically change these code chunk options over with the following code.

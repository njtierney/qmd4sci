...
* Why won't my document render?
* How do I read a Quarto error message?
* How do I fix the errors I am most likely to hit?
...
-   Recreate the error in an interactive session:
    ...
    -   run all chunks above and below
    -   explore working directory issues
        -   remember that the Quarto directory is where the .qmd file lives

## The errors

What follows from here are all the errors you might find in a Quarto document, with the following structure:
...
This is caught before the document compiles with an error like:
...
2.  Run code above the problem chunk. In RStudio that is the grey downward arrow at the top right of the chunk, or Run \> Run All Chunks Above
...
The document renders to HTML without complaint, but rendering to PDF stops. The message usually names either a `.sty` file you have never heard of, or a character that LaTeX cannot typeset.

**The error message**

```
compilation failed- no matching packages
LaTeX Error: File `sansmathaccent.sty' not found.
```

or

```
! Package inputenc Error: Unicode character ° (U+00B0)
(inputenc)                not set up for use with LaTeX.
```

**How to solve**

-   For a missing `.sty` file, render again first. Quarto installs missing LaTeX packages as it goes, so the second attempt often succeeds. If it does not, install it yourself with `tinytex::tlmgr_install("sansmathaccent")`, using whichever name the message gave you.
-   For a Unicode character, either write it the LaTeX way (`$^\circ$C` instead of `°C`), or switch to an engine that handles Unicode by adding `pdf-engine: xelatex` to your YAML.
-   If you are stuck, render to HTML. If the HTML is fine, the problem is LaTeX rather than your code, which is a much smaller thing to go looking for.
...
You create a figure in a chunk, label the chunk `fig-something`, and refer to it in text with `@fig-something`. The reference stays as the literal text `@fig-something` instead of turning into "Figure 1".

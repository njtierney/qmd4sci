...
#| fig-cap: "Life expectancy from 1952 - 2007 for Australia. Life expectancy increases steadily except from 1962 to 1969. We can safely say that our life expectancy is higher than it has ever been!"
library(ggplot2)
library(dplyr)
...
Sometimes you want to add multiple figures that are linked, or slightly different views of similar data. Quarto calls these **subfigures**. You give the chunk one `fig-cap` for the whole figure, and a `fig-subcap` with one caption per panel. You can then reference the individual panels with `@fig-<chunk-name>-1` and `@fig-<chunk-name>-2`. For example:

```{r}
#| label: fig-volcanos
#| layout-ncol: 2
#| echo: fenced
#| fig-cap: "Two views of Auckland's Maunga Whau Volcano"
#| fig-subcap:
#|   - "An image plot"
#|   - "A contour plot"
...
The panels get lettered captions, (a) and (b). We can see the image plot as (`@fig-volcanos-1`) @fig-volcanos-1 and the contour plot as (`@fig-volcanos-2`) @fig-volcanos-2. The figure as a whole is (`@fig-volcanos`) @fig-volcanos.

::: {.callout-warning title="A list in `fig-cap` works, too!" collapse="true"}

You can give `fig-cap` a list of captions instead, one per plot. That  does not make subfigures, but produces two separate figures, numbered Figure 1 and Figure 2, each with its own caption, and `@fig-volcanos` on its own no longer resolves - you only get the numbered references, `@fig-volcanos-1` and `@fig-volcanos-2`.
...
1. Download the gapminder logo, which is at [this link](https://www.gapminder.org/wp-content/themes/gapminder2/images/gapminder-logo.svg)
1. Put it into a new directory called "figs" (at the same level as your `qmd4sci-exercises.Rproj` file)

...
```` markdown
```{{python}}
py_heights = [153, 151, 156, 160, 171]
import statistics
py_mean = statistics.mean(py_heights)
```

The mean of these heights is `{python} py_mean`
````

This syntax works for any Jupyter kernel---so for Julia you would write an inline expression as `` `{julia} j_mean` ``).
...
The mean of these heights is `{r} mean(c(153, 151, 156, 160, 171))`
````

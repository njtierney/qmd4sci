...
-   Tables are referenced in text with `@tbl-label` (e.g., `@tbl-gapminder`)
-   Figures are references in text with `@fig-label`(e.g., `@git-gapminder`)
...
See above in Table [-@tbl-gg-oz-tab].
...
You must also include `number-sections` in your YAML:
...
For example, I can refer to the first section `(@sec-start)` in this document by referring to the section as
...
2.  Open up "04-qmd-global-opts-device.qmd" and work through the tasks there
:::

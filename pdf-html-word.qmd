...
Rendering is also not how you find out whether your code works. Run the chunks first, fix whatever breaks, and then render. Rendering starts a fresh R session and runs your code from the top, so it is the slowest way there is to discover a typo.
...
1. Using your “01-qmd-example.qmd” Quarto document you were using earlier, generate three reports, one as HTML, one as PDF, and one as microsoft word. You can render with the Render button, or with `Ctrl + Shift + K` on Windows and `Cmd + Shift + K` on a Mac. Remember, if you are having PDF problems, see the [installation chapter](installation) note on installing LaTeX with the R package, `tinytex`.

...
We get a clue that the warning is coming from the line `mean_color`, so let's just try and do that line:
...
We still get that warning, and an `NA`, so what if we just do

```{r}
mean(diamonds$color)
```

OK, same warning. What is in color?
